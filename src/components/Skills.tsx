...
function SkillPill(props: SkillPillProps) {
  ...
  return (
    <div className="flex w-full items-center gap-2 overflow-hidden rounded-lg border border-tera-500/20 bg-white px-4 py-3 text-sm shadow-sm dark:bg-zinc-800 sm:text-base md:px-6 md:py-3 md:text-lg">
      <span className="relative h-5 w-5 sm:h-8 sm:w-8 flex items-center">
        {icon}
      </span>
  ...
  return (
    <section className="overflow-hidden px-6 py-32 sm:px-14 md:px-20">
      <div className="relative mx-auto max-w-7xl">
        ...
        <AnimatePresence>
          <div className="mt-4">
            <span className="text-xs font-semibold sm:text-sm">Devops</span>
            <div className="mt-2 flex flex-wrap gap-4 text-xl dark:text-zinc-100">
              {DEVOPS.map((pill, index) => (
                <FadeRight
                  key={`lang-${index}`}
                  duration={0.4}
                  delay={0.1 + index * 0.1}
